...
void VoEExtendedTest::CallbackOnOutputAudioRouteChange(const OutputAudioRoute route) {
  switch (route)
  {
      ...
      case webrtc::kOutputAudioRouteBuiltInReceiver:
          printf("Built in receiver\n");
          break;
      case webrtc::kOutputAudioRouteBuiltInSpeaker:
          printf("Built in speaker\n");
          break;
  }
}
...
int VoEExtendedTest::TestCodec(const char* sndIpAddress, int sndPort, int rcvPort) {
  PrepareTest("Codec");
...
//  TEST_MUSTPASS(voe_base_->Init());
  TEST_MUSTPASS(voe_base_->CreateChannel());
  TEST_MUSTPASS(voe_base_->RegisterVoiceEngineObserver(*this));
  TEST_MUSTPASS(apm->SetEcStatus(true, webrtc::kEcAecm));
  TEST_MUSTPASS(apm->SetAecmMode(webrtc::kAecmEarpiece));
  TEST_MUSTPASS(apm->SetAgcStatus(true, webrtc::kAgcAdaptiveDigital));
//  TEST_MUSTPASS(apm->SetNsStatus(true, webrtc::kNsLowSuppression));
  ...
  TEST_MUSTPASS(voe_base_->SetLocalReceiver(0, 12345));
  TEST_MUSTPASS(voe_base_->SetSendDestination(0, 12345, "127.0.0.1"));
//  TEST_MUSTPASS(voe_base_->SetSendDestination(0, sndPort, sndIpAddress));
//  TEST_MUSTPASS(voe_base_->SetLocalReceiver(0, rcvPort));
#endif
  TEST_MUSTPASS(voe_base_->StartReceive(0));
  TEST_MUSTPASS(voe_base_->StartSend(0));
  TEST_MUSTPASS(voe_base_->StartPlayout(0));
...
//  cinst.channels = 1; 
  ...
  CodecInst cinst_file;
  strcpy(cinst_file.plname, "VORBIS");
  cinst_file.pltype = 109;
  cinst_file.rate = 32000;
  cinst_file.pacsize = 480; // 30ms
  cinst_file.plfreq = 16000;
  cinst_file.channels = 1;    

//  TEST_MUSTPASS(file->StartRecordingCall(GetFilename("test.ogg"), &cinst_file));

  codec->SetSendCodec(0, cinst);
  TEST_LOG("[%2d] %16s: fs=%6d, pt=%4d, rate=%7d, ch=%2d, size=%5d",
           cIndex, cinst.plname, cinst.plfreq, cinst.pltype, cinst.rate,
...
/*
  ...
  volume->SetInputMute(-1, true);
  volume->GetInputMute(-1, muteEnabled);
  printf("Mute: %s\n", muteEnabled ? "enabled" : "disabled");

  SLEEP(10000);
  
  volume->SetInputMute(-1, false);
  volume->GetInputMute(-1, muteEnabled);
  printf("Mute: %s\n", muteEnabled ? "enabled" : "disabled");
*/
  SLEEP(10000);
  ...
  hardware->GetLoudspeakerStatus(loudspeakerEnabled);
  printf("Loudspeaker: %s\n", + loudspeakerEnabled ? "enabled" : "disabled");
  ...
  TEST_MUSTPASS(file->StopRecordingCall());
...
void VoEExtendedTest::Print(const TraceLevel level, const char *traceString, const int length)
{
    printf("WEBRTC LOG: %s\n", traceString);
}
